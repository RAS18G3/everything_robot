...
void ObjectFilterNode::process_data() {
  ...
  if(points_in_camera_ > threshold_ && last_3d_pointcloud_msg_ != nullptr) {
    ...
    pcl_ros::transformPointCloud("/map", pcl_conversions::fromPCL(last_3d_pointcloud_msg_->header.stamp), *last_3d_pointcloud_msg_, "/camera_depth_frame", transformed_pointcloud, tf_listener_);
        ...
        if(count > 5) {
          ...
          for(auto object_it = objects_.begin(); object_it != objects_.end(); ++object_it) {
            if(std::sqrt(std::pow(object_it->position.x - x_avg, 2) + std::pow(object_it->position.y - y_avg, 2)) < object_distance_) {
              ...
              ROS_INFO_STREAM("Old object: " << objects_.back().position.x << " " << objects_.back().position.y << " " << most_likely_class);
          ...
          if(new_object) {
            ...
            ROS_INFO_STREAM("New object: " << x_avg << " " << y_avg << " " << class_id);
          }
        }
      }
    }
